...
class ResilientSocketFactory(
    ...
    override fun createSocket(): Socket {
        val target = delegate ?: return defaultFactory.createSocket()
        return try {
            target.createSocket()
        } catch (t: Throwable) {
            if (isPermOrBindingFailure(t)) {
                AppLogger.w("ResilientSocket", "createSocket() failed with ${t.message}; falling back to default socket", t)
    ...
    override fun createSocket(host: String, port: Int): Socket {
        val target = delegate ?: return defaultFactory.createSocket(host, port)
        return try {
            target.createSocket(host, port)
        } catch (t: Throwable) {
            if (isPermOrBindingFailure(t)) {
                AppLogger.w("ResilientSocket", "createSocket($host:$port) failed with ${t.message}; falling back to default socket", t)
